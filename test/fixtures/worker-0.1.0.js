...
var require_tree_sitter2 = __commonJS2({
  "node_modules/web-tree-sitter/tree-sitter.cjs"(exports, module) {
    "use strict";
...
var DEFAULT_LOGICAL_PARENTS = /binary|boolean_operator|infix|conjunction|disjunction|logical/;
var C_FAMILY_FLOW = {
  if: ["if_statement"],
  else: ["else_clause"],
...
function elixirTarget(node) {
  if (node.type !== "call") return void 0;
  const t = node.childForFieldName("target");
  return t ? t.text : void 0;
}
var ELIXIR_FN = /* @__PURE__ */ new Set(["def", "defp", "defmacro", "defmacrop"]);
var ELIXIR_CLASS = /* @__PURE__ */ new Set(["defmodule", "defprotocol", "defimpl"]);
var ELIXIR_IF = /* @__PURE__ */ new Set(["if", "unless"]);
var ELIXIR_SWITCH = /* @__PURE__ */ new Set(["case", "cond", "with", "receive", "try"]);
var ELIXIR_LOOP = /* @__PURE__ */ new Set(["for"]);
function elixirHead(node) {
  for (let i2 = 0; i2 < node.namedChildCount; i2++) {
    const c = node.namedChild(i2);
    if (c.type === "arguments") {
      const first = c.namedChild(0);
      if (!first) return void 0;
      if (first.type === "binary_operator") return first.namedChild(0) ?? void 0;
      return first;
    }
  }
  return void 0;
...
var TIER1 = [
  ...
  },
  {
    ...
    nameOf(node) {
      if (node.type !== "function_body") return void 0;
      ...
      if (n) return n.text;
      for (let i2 = 0; i2 < inner.namedChildCount; i2++) {
        const c = inner.namedChild(i2);
  ...
  },
  {
    ...
    roles: {
      ...
      catch: ["rescue_block", "catch_block"],
      case: ["stab_clause"]
    },
    customRole(node) {
      ...
      if (t === void 0) {
        if (node.type === "stab_clause") {
          ...
          const ot = owner ? elixirTarget(owner) : void 0;
    ...
    paramsOf(node) {
      ...
      for (let i2 = 0; i2 < head.namedChildCount; i2++) {
        const c = head.namedChild(i2);
  ...
  },
  {
    ...
    roles: {
      ...
      if: ["if_statement"],
      elseif: ["elseif_clause", "elseif_statement"],
      else: ["else_clause", "else_statement"],
  ...
  },
  {
    ...
    customRole(node) {
      if (node.type === "let_binding") {
        for (let i2 = 0; i2 < node.namedChildCount; i2++) {
          if (node.namedChild(i2).type === "parameter") return "function";
        }
        return null;
      }
      return void 0;
    ...
    paramsOf(node) {
      if (node.type !== "let_binding") return void 0;
      let n = 0;
      for (let i2 = 0; i2 < node.namedChildCount; i2++) if (node.namedChild(i2).type === "parameter") n++;
      return n;
    },
  ...
  },
  {
    ...
    roles: {
      ...
      loop: ["for_expression", "while_expression"],
      switch: ["switch_expression", "try_expression"],
      case: ["switch_match"],
      ternary: ["ternary_expression"]
    }
  },
...
for (const spec of TIER1) {
  ...
  for (const [role, types] of Object.entries(spec.roles)) {
...
var T2 = (key, display, comments, exts) => ({ key, display, comments, exts });
var TIER2_LIST = [
  T2("elm", "Elm", ["--"], [".elm"]),
...
function tier2Info(key) {
  const name2 = key.startsWith("tier2:") ? key.slice(6) : key;
  return TIER2.get(name2) ?? { key: name2, display: name2.toUpperCase(), comments: ["//", "#"] };
}
...
var NON_OPERATORS = /* @__PURE__ */ new Set([")", "]", "}", ",", ";", '"', "'", "`", "\n", ""]);
...
var isComment = (type) => type.includes("comment");
function tokenPass(cursor, lineCount) {
  const tokens = [];
  const codeRows = new Uint8Array(lineCount);
  const commentRows = new Uint8Array(lineCount);
  const todos = [];
  let descend = true;
  for (; ; ) {
    ...
    if (descend) {
      if (isComment(type) && cursor.nodeIsNamed) {
        ...
        for (let i2 = 0; i2 < lines.length; i2++) {
          const m = TODO_RE.exec(lines[i2]);
          if (m) todos.push({ line: s + i2, col: i2 === 0 ? cursor.startPosition.column + m.index : m.index, text: (m[1] + " " + m[2]).trim().slice(0, 120) });
        }
        descend = false;
        continue;
      }
...
function discoverUnits(root, spec) {
  const units = [];
  const classes = [];
  const visit = (node, ctxUnit, ctxClass) => {
    for (let i2 = 0; i2 < node.namedChildCount; i2++) {
      const c = node.namedChild(i2);
      const r = roleOf(spec, c);
      if (r === "function" || r === "lambda" && !ctxUnit) {
        const u = { node: c, kind: r === "lambda" ? "lambda" : ctxClass ? "method" : "function", nested: [] };
        if (ctxClass && r === "function") ctxClass.methods++;
        if (ctxUnit) ctxUnit.nested.push([c.startIndex, c.endIndex]);
        units.push(u);
        visit(c, u, null);
      } else if (r === "class") {
        const cls = { node: c, methods: 0 };
        classes.push(cls);
        if (ctxUnit) ctxUnit.nested.push([c.startIndex, c.endIndex]);
        visit(c, null, cls);
      } else {
...
var LOGICAL_WORDS = /* @__PURE__ */ new Set(["&&", "||", "and", "or"]);
var LOGICAL_NAMED = /* @__PURE__ */ new Set(["logical_and_operator", "logical_or_operator"]);
var DEFAULT_CASE_RE = /^(default\b|else\b|case\s+_\s*(=>|:|if\b)|_\s*(=>|->)|\|\s*_\s*(=>|->)|true\s*->)/;
function logicalInfo(node, spec) {
...
function isDefaultCase(node) {
  ...
  if (pat && pat.text.trim() === "_") return true;
  return DEFAULT_CASE_RE.test(node.text.trimStart());
}
function unwrap(node, spec) {
  let n = node;
  while (n && spec.transparent?.includes(n.type) && n.namedChildCount === 1) n = n.namedChild(0);
  return n;
}
function complexityWalk(unit, spec) {
  let cc = 1, cog = 0, maxNest = 0;
  const elseIfIds = /* @__PURE__ */ new Set();
  const scanElseKeywords = (ifNode) => {
    for (let i2 = 1; i2 < ifNode.childCount; i2++) {
      const c = ifNode.child(i2);
  ...
  const elseBelongsToIf = (elseNode) => {
...
function halstead(tokens, from, to, nested) {
  const ops = /* @__PURE__ */ new Map(), opnds = /* @__PURE__ */ new Map();
  let N1 = 0, N2 = 0;
  let lo = 0, hi = tokens.length;
  while (lo < hi) {
    const mid = lo + hi >> 1;
    if (tokens[mid].start < from) lo = mid + 1;
    else hi = mid;
  }
  for (let i2 = lo; i2 < tokens.length && tokens[i2].start < to; i2++) {
    const t = tokens[i2];
    if (nested.some(([s, e]) => t.start >= s && t.start < e)) continue;
    if (t.named) {
      N2++;
      opnds.set(t.text, (opnds.get(t.text) ?? 0) + 1);
    } else if (!NON_OPERATORS.has(t.text)) {
      N1++;
      ops.set(t.text, (ops.get(t.text) ?? 0) + 1);
    }
  }
...
function maintainabilityIndex(volume, cc, sloc) {
...
var NAME_LEAF_RE = /^(identifier|simple_identifier|name|word|constant|field_identifier|property_identifier|type_identifier|qualified_identifier|operator_name|destructor_name|scoped_identifier)$/;
var PARAM_LIST_RE = /^(parameters|parameter_list|formal_parameters|formal_parameter_list|method_parameters|function_value_parameters|lambda_parameters|closure_parameters|block_parameters|parameter_clause|function_parameters)$/;
var PARAM_ITEM_RE = /^(parameter|formal_parameter|simple_parameter|lambda_parameter|class_parameter)$/;
var clean = (s) => s.split("\n")[0].trim().slice(0, 60);
function nameOf(node, spec) {
  ...
  for (let i2 = 0; i2 < node.namedChildCount; i2++) {
    const c = node.namedChild(i2);
    if (c === body2 || c.id === body2?.id) break;
...
function paramsOf(node, spec) {
  const custom = spec.paramsOf?.(node);
  if (custom !== void 0) return custom;
  if (node.childForFieldName("parameter")) return 1;
  const body2 = node.childForFieldName("body");
  const queue = [[node, 0]];
  while (queue.length) {
    ...
    for (let i2 = 0; i2 < n.namedChildCount; i2++) {
      ...
      if (body2 && c.id === body2.id) continue;
  ...
  let direct = 0;
  for (let i2 = 0; i2 < node.namedChildCount; i2++) if (PARAM_ITEM_RE.test(node.namedChild(i2).type)) direct++;
  return direct;
}
function analyzeTier1(root, cursor, specKey, lineCount) {
  ...
  const classMetrics = classes.map((c) => ({
    ...
    startLine: c.node.startPosition.row,
  ...
  tp.tokens.forEach((t, i2) => {
    tokenIds[i2] = fnv1a(t.text);
    tokenLines[i2] = t.row;
  });
  return {
    functions,
...
var TOKEN_RE = /[A-Za-z_$][\w$]*|\d+(?:\.\d+)?|"(?:[^"\\\n]|\\.)*"|'(?:[^'\\\n]|\\.)*'|\S/g;
var TODO_RE2 = /\b(TODO|FIXME|HACK|XXX)\b[:\s-]*(.*)/;
var NO_INDENT_NESTING = /* @__PURE__ */ new Set(["html", "css", "vue", "svelte", "xml"]);
function analyzeTier2(text, commentPrefixes, languageKey) {
  ...
  const indents = [];
  const deltaVotes = /* @__PURE__ */ new Map();
  let prevIndent = 0;
  for (let i2 = 0; i2 < lines.length; i2++) {
    const raw = lines[i2];
    const trimmed = raw.trim();
    if (!trimmed) continue;
    const todo = TODO_RE2.exec(raw);
    if (todo) todos.push({ line: i2, col: todo.index, text: (todo[1] + " " + todo[2]).trim().slice(0, 120) });
    if (commentPrefixes.some((p) => trimmed.startsWith(p))) {
      commentLines++;
      continue;
    }
    sloc++;
    ...
    indents.push(w);
    if (w > prevIndent) {
      const d = w - prevIndent;
      deltaVotes.set(d, (deltaVotes.get(d) ?? 0) + 1);
    }
    prevIndent = w;
  ...
  let unit = 4, best = 0;
  for (const [d, votes] of deltaVotes) if (d >= 2 && d <= 8 && votes > best) {
    best = votes;
    unit = d;
  }
  let indentNesting;
  const name2 = languageKey.replace(/^tier2:/, "");
  if (!NO_INDENT_NESTING.has(name2)) {
  ...
  return {
    ...
    lineLengths,
    indentNesting,
    tokenIds: Uint32Array.from(ids),
    tokenLines: Uint32Array.from(tokLines)
  };
}
...
var import_web_tree_sitter = __toESM2(require_tree_sitter2());
var GrammarError = class extends Error {
  constructor(grammar, message) {
    super(message);
...
var ParserService = class {
  ...
  constructor(wasmDir) {
    ...
    this.broken = /* @__PURE__ */ new Set();
  }
  /** Initialize the WASM runtime exactly once (idempotent). */
  init() {
    this.ready ??= import_web_tree_sitter.Parser.init({
      // WHY: bundlers move files around; point the runtime at our copy.
      locateFile: (name2) => path.join(this.wasmDir, name2)
    });
    return this.ready;
  }
  isBroken(key) {
    return this.broken.has(key);
  }
  ...
  async parserFor(key) {
    ...
    try {
      const bytes = fs2.readFileSync(path.join(this.wasmDir, `tree-sitter-${key}.wasm`));
      const lang = await import_web_tree_sitter.Language.load(bytes);
      ...
      this.broken.add(key);
      throw new GrammarError(key, `failed to load grammar ${key}: ${String(e?.message ?? e)}`);
    }
  }
  /** Parse text. Caller MUST call tree.delete() (WASM memory is manual). */
  async parse(key, text) {
    ...
    try {
      ...
      this.broken.add(key);
      throw new GrammarError(key, `parse failed for ${key}: ${String(e?.message ?? e)}`);
    }
  }
};
...
var GENERATED_HEADER = /auto-?generated|do not edit|@generated|generated by/i;
var GENERATED_PATH = /(\.min\.(js|css)$)|(\.pb\.go$)|(_pb2(_grpc)?\.py$)|(\.g\.dart$)|(\.freezed\.dart$)|(\.designer\.cs$)|(\.generated\.\w+$)/i;
function skipped(languageKey, reason, lines = 0) {
  return {
    ...
    classes: [],
    todos: [],
    lineLengths: new Uint32Array(0),
    hasSyntaxErrors: false,
    tokenIds: new Uint32Array(0),
    tokenLines: new Uint32Array(0),
    analysisMs: 0
  };
}
function looksMinified(text) {
  if (text.length < 2e3) return false;
  const lines = text.split("\n");
  const avg = text.length / lines.length;
  return avg > 500 || lines.some((l) => l.length > 5e3);
}
function notebookToPython(text) {
  try {
    ...
    (nb.cells ?? []).forEach((cell, idx) => {
      ...
      for (const line of src.split("\n")) {
        out2.push(/^\s*[%!]/.test(line) ? "# " + line : line);
        cellOfLine.push(idx + 1);
      }
    });
    return { code: out2.join("\n"), cellOfLine };
  } catch {
    return void 0;
  }
}
async function analyze(parser, languageKey, filePath, text, forceGeneric = false) {
  const t0 = Date.now();
  ...
  if (getSpec(grammarKey) && !parser.isBroken(grammarKey) && !forceGeneric) {
    ...
    try {
      ...
      try {
        const m = analyzeTier1(tree.rootNode, cursor, grammarKey, lines);
        const result = {
          engineVersion: ENGINE_VERSION,
          languageKey,
          tier: 1,
      ...
      fallbackNote = `Full analysis failed (${String(e?.message ?? e)}); showing partial analysis.`;
    } finally {
      tree?.delete();
    }
  ...
  const info2 = tier2Info(languageKey.startsWith("tier2:") ? languageKey : "tier2:" + grammarKey);
  const g = analyzeTier2(source, info2.comments, languageKey);
  return {
    engineVersion: ENGINE_VERSION,
    languageKey,
    tier: 2,
    lines: g.lines,
    sloc: g.sloc,
    commentLines: g.commentLines,
    functions: [],
    classes: [],
    todos: g.todos,
    lineLengths: g.lineLengths,
    indentNesting: g.indentNesting,
    hasSyntaxErrors: false,
    tokenIds: g.tokenIds,
    tokenLines: g.tokenLines,
    analysisMs: Date.now() - t0,
    fallbackNote
  };
}
...
// src/engine/worker.ts
var service = new ParserService(import_worker_threads.workerData.wasmDir);
void service.init();
import_worker_threads.parentPort.on("message", async (req) => {
  try {
    const analysis = await analyze(service, req.languageKey, req.path, req.text, req.forceGeneric);
    const res = { id: req.id, ok: true, analysis };
    import_worker_threads.parentPort.postMessage(res, [analysis.tokenIds.buffer, analysis.tokenLines.buffer, analysis.lineLengths.buffer]);
  } catch (e) {
    const res = {
      id: req.id,
      ok: false,
      error: String(e?.message ?? e),
      brokenGrammar: e instanceof GrammarError ? e.grammar : void 0
    };
    import_worker_threads.parentPort.postMessage(res);
  }
});
